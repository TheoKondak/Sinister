export class FullScreenMenuGrid {
  init(
    ...
    const { background, containerSelector, backgroundContainerSelector, scaleInDelay, fadeAnimationDelay, defaultImage } = data; // destructure data
    const moduleContainer = document.querySelector(containerSelector);
    let backgroundImageContainer = moduleContainer.querySelector(backgroundContainerSelector);
    ...
    menuItemsContent.forEach((menuItem) => {
      ...
      for (const menuItemId in background) {
        const currentMenuKey = Object.keys(background[menuItemId]);
        ...
        if (menuItem.replace(/\s+/g, '-').toLocaleLowerCase() === currentMenuKey[0].replace(/\s+/g, '-').toLocaleLowerCase()) {
          ...
          backgroundImageContainer.insertBefore(newImage, document.querySelector(defaultImage)).classList.add('menu-grid--background', 'hidden', menuItem.replace(/\s+/g, '-').toLocaleLowerCase());
        }
        ...
        document.querySelector('.' + menuItem).addEventListener('mouseenter', (e) => {
          ...
          }, fadeAnimationDelay); // set a delay in ms
